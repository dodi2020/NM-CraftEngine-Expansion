...
module.exports.transform = (item, context) => {
    ...
    const getStateConfig = () => {
        ...
        if (item.modules?.craftengine_stateAuto) {
            ...
            // Default: use auto-state with material type (note_block, mushroom_stem, etc.)
            const material = (item.modules?.baseMaterial || 'note_block').toLowerCase();
            stateConfig['auto-state'] = material;
        }
    ...
    // Add state configuration (single-state blocks) - REQUIRED for all blocks
    const stateConfig = getStateConfig();
    transformer.state = stateConfig;

    // Add states configuration (multi-state blocks) - overrides single state if present
    const statesConfig = getStatesConfig();
    if (statesConfig) {
        delete transformer.state; // Remove single state if multi-state is defined
...
module.exports.untransform = (exportedData) => {
    ...
    // Map behavior field (single behavior)
            ...
            if (typeof value === 'object' && !Array.isArray(value)) {
                ...
                for (const [k, v] of Object.entries(value)) {
                    if (typeof v === 'object' && !Array.isArray(v)) {
                        ...
                        behaviorYaml.push(`  ${k}:`);
                        v.forEach(item => behaviorYaml.push(`    - ${item}`));
                    } else {
    ...
    if (exportedData.state) {
        const state = exportedData.state;
        if (state['auto-state']) modules.craftengine_stateAuto = state['auto-state'];
        if (state['state']) modules.craftengine_state = state['state'];
        if (state['id'] !== undefined) modules.craftengine_stateId = state['id'];
    }
    
    // Map states configuration (multi-state)
    if (exportedData.states) {
        const states = exportedData.states;
        if (states.properties) modules.craftengine_statesProperties = states.properties;
        if (states['id'] !== undefined) modules.craftengine_statesId = states['id'];
    }
